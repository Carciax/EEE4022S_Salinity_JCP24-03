...
Following this introduction in \refch{ch:introduction}, the literature review in \refch{ch:literature} will give an overview of salinity, the methods of measuring it, and the devices used to measure it.
The method of calculating salinity is cover in \refch{ch:theory-development}.
This is followed by the design of the probe circuitry, its electrodes and a controller required to measure salinity and a controller  in \refch{ch:design}.
\refch{ch:implementation} details the fabrication of the designed probe circuitry and the controller onto \glspl{pcb}, the correction made to them, and the waterproofing of the probe by casting it into epoxy.
\refch{ch:testing} details the evaluation of the probe's resistance measurement accuracy, following by an investigation into and evolution of a method to measure conductivity and to calculate salinity.
Finally, \refch{ch:conclusions} concludes the report with a summary of the project's objectives, the results obtained, and \refch{ch:recommendations} provides recommendations for future work.