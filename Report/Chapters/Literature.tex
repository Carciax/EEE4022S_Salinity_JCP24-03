...
Around the same time as the salinity-chlorinity relationship was established, oceanographers had begun experimenting with the use of conductivity to measure salinity.
...
Calculating salinity from conductivity was however not as straight forward requiring several equations which are discussed in~\refsec{sec:salinity-conductivity-relationship}.
\textit{accuracy achieved?, device?, limitations?}
...
The relationship between salinity and density can be estimated to be linear by \refeqn{eq:salininty-density} where $\rho$ is the density of the water, $\rho_0$ is the density of pure water, $k$ is a proportionality constant, and $S$ is the salinity of the water~\cite{kjerfve_salinity_measurement_overview_1983}\cite{uow_oceanography_research_1966}.
\begin{equation}\label{eq:salinity-density}
    \rho = \rho_0(1 + kS)
\end{equation}
This relationship was further refined by~\refref{schmidt_density_salinity_relation_2018} which included temperature variation in the relationship.
\refref{schmidt_density_salinity_relation_2018} also claimed that the relationship between salinity and density was a better measure of salinity than conductivity as the standard potassium chloride solution used to calibrate the conductivity meters did not account for the other ions commonly present in salt water while the density of the water did.
\textit{accuracy achieved?, device?, limitations?} 
...
This has allowed for the development of satellites that can measure the salinity which have been used to develop global salinity maps as shown in \reffig{}.
%https://www.geeksforgeeks.org/salinity-of-ocean-water-factors-horizontals-and-vertical-distribution/
...
Salt water interacts with the entire electromagnetic spectrum including visible light where it refracts or scatters the light in a way that is dependent on the temperature, pressure, and more importantly the salinity of the water.
The relationship between salinity and refractive index is complex requiring a 27 term equation which relates the refractive index of the water, its pressure, and its temperature to its salinity.
The refractive index equation is defined a range of $500-700nm$ in wave length, $0-30\degree C$ in temperature, and $0-40$ on the Practical Salinity Scale PSS-78, and $0-11000 dbar$ in pressure.~\cite{millard_index_of_refraction_algorithm_1990}
The equations holds an accuracy of $0.4-80$ \gls{ppm} on the Practical Salinity Scale PSS-78, decreasing with increasing pressure. 
...
There are a few researches who have developed compact refractometers which have dimensions of $22.5mm \times 22.5mm \times 120mm$~\cite{malarde_compact_refractometer_2008} and $40mm \times 40mm \times 70mm$~\cite{tengesdal_compact_refractometer_2012} which achieved accuracies of 2 and 83 \gls{ppt} Practical Salinity Scale PSS-78 respectively.
...
The conductivity measurement taken in the field $C(S_p, t, p)$ is related to the conductivity of the standard solution $C(35, 15\degree C, 0)$ which the device is calibrated with and is represented by $R$ in \refeqn{eq:pss-78-full-ratio}.~\cite{ioc_teos_2010}
